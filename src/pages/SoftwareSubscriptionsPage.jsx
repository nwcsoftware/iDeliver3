...
export default function SoftwareSubscriptionsPage() {
  ...
  function openPayments(r) { setPayFor(r); setPayForm(null); setPayErr('') }
      ...
      {payFor && (
        <div className="card overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-surface-border bg-surface-hover/40">
            ...
            <button onClick={() => setPayFor(null)} className={`btn-ghost p-1.5 text-slate-500 hover:text-slate-200 ${isSuperAdmin ? '' : 'ml-auto'}`}>
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border">
                {['Paid on', 'Amount', 'Covers until', 'Method / reference', 'Confirmed', ''].map((h, i) => (

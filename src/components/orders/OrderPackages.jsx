...
import { buildTrackingNumber } from '../../lib/orderPackages'
...
export default function OrderPackages({ packages, setPackages, providers = [], onAddProvider, customerName = '', embedded = false, onAdd, hideProvider = false, fixedProviderName = '' }) {
  ...
  // Every package carries a tracking number. Whoever created the row — the
  // section header, this subform, or an older order saved before numbers were
  // issued — anything still blank gets one here, so the field is never empty.
  React.useEffect(() => {
    if (!packages.some(p => !p.tracking_number?.trim())) return
    setPackages(list => {
      const issued = list.map(p => p.tracking_number).filter(Boolean)
      let changed = false
      const next = list.map(p => {
        if (p.tracking_number?.trim()) return p
        const tracking_number = buildTrackingNumber(issued)
        issued.push(tracking_number)
        changed = true
        ...
        <div key={p._id ?? p._key ?? i} data-package-row className="border border-surface-border rounded-lg p-3 space-y-2 bg-surface-hover/30">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              {/* System-issued (date-time-milliseconds) and read-only: it is the
                  package's identity, printed on labels and quoted by drivers and
                  partners, so it must never change once the row exists. */}
              <input readOnly title="Generated by the system — it can't be changed"
                className="input flex-1 font-mono cursor-not-allowed opacity-90 focus:ring-0 focus:border-surface-border"
